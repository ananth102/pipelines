...
class EndpointComponentTestCase(unittest.TestCase):
    ...
    def test_get_job_status(self):
        with patch(
            ...
            # InService Endpoint
            mock_get_resource.return_value = {"status": {"endpointStatus": "InService"}}
    ...
    def test_get_upgrade_status(self):
        with patch(
            ...
            with patch(
                ...
                mock_condition_list.return_value = [
                    "Endpoint is in Updating state cannot be modified or deleted"
                ]
                self.assertEqual(
                    self.component._get_upgrade_status(),
                    SageMakerJobStatus(
                        is_completed=False, raw_status="InService", has_error=False
                    ),
                )
